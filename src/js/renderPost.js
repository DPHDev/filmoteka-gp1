...
export async function renderPost(posts, page, listGenres) {
  try {
    ...
    const promises = arrayMovies.map(
      async ({
        poster_path,
        id,
        title = 'N/N',
        ...
        // Genres list charge
        const genres = await varDOM
          .genresList(genre_ids, listGenres)
          .map(elemento => {
            return ' ' + elemento;
          });
        // Delimitation of the list of genres and date
        const genresDel =
          genres.length > 2 ? `${genres.slice(0, 2)}, Others` : genres;
        const date_year =
          release_date != undefined ? release_date.slice(0, 4) : '2023';

        const poster =
          poster_path === null
            ? varDOM.defaultPoster
...
export function printCard(results) {
  ...
  insertCard = `
    ...
    results.poster_path
  }" alt="${results.tittle}" loading="lazy"  data-id="${results.id}"/>
                        </a>
                        <figcaption class="info">
