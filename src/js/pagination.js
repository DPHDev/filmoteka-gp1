import { total_pgs, pageNow } from "./renderPost";
...
export function paginationButtons() {
    ...
    if (total_pgs > 9) {
        ...
        markup += `<li><button>...</button></li>
        <li><button>${total_pgs}</button></li>`;
...
export function renderPerPagination() {
    ...
    if (event.target && event.target.matches("#num-page-btn")) {
        ...
        const posts_per_page = await getAPI.movies(varDOM.movieName.value.trim(), page.textContent);
        await renderPost(posts_per_page, page.textContent, mListGen);
        paginationButtons();
        ...
        const mListGen = await getAPI.genres();
        const posts_per_page = await getAPI.movies(varDOM.movieName.value.trim(), page_adv);
        ...
        const mListGen = await getAPI.genres();
        const posts_per_page = await getAPI.movies(varDOM.movieName.value.trim(), page_back);
